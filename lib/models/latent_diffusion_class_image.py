...
class LatentDiffusionClassImage(BaseModel):
    ...
    def train_step(self, data, optimizer, loss_scaler=None, running_status=None):
        ...
        grad_accum_steps = self.train_cfg.get('grad_accum_steps', 1)
        # Identify micro-step positions within the accumulation window
        is_last_micro = ((running_status['iteration'] + 1) % grad_accum_steps) == 0
        is_first_micro = (running_status['iteration'] % grad_accum_steps) == 0
        ...
        # Avoid gradient all-reduce on non-final micro-steps when under DDP
        ddp_ctx = (self.no_sync() if (not is_last_micro and hasattr(self, 'no_sync')) else nullcontext())
        ...
        with ddp_ctx:
            ...
            # Scale loss across accumulation steps and backpropagate
            loss = loss / grad_accum_steps
            (loss if loss_scaler is None else loss_scaler.scale(loss)).backward()

        # Perform optimizer step at the end of the accumulation window
        if is_last_micro:
